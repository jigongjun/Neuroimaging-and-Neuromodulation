function alignLandmarks = TMSviewer(fileNameList, outFileName, alignLandmarks, newMmPerVox, weights, bb, showFigs, clipVals)
%
...
%
% fileNameList=[pwd,'/*.nii.gz']
%        dir(fullfile(fileNameList))
...
%     end
%     fileNameList = tmp;        %% Initialize parameters
...
if ( ischar(fileNameList) )
    ...
    for ii=1:length(d)
        tmp{ii} = fullfile(mrvDirup(fileNameList),d(ii).name);
%         tmp{ii} = fullfile(fileNameList,d(ii).name); % jgj modified
...
if(isempty(alignLandmarks))
    ...
    h = figure('unit','normal','pos', [0.18 0.08 0.7 0.85],'name','Set AC-PC landmarks');
    if ~isnumeric(h)
